// Package i18n provides the locale-aware message translation seam for
// the Plugins submodule. It is the CONST-046-compliant indirection
// between production code in pkg/* and the locale bundles under
// pkg/i18n/bundles/.
//
...
// default), and the Plugins submodule never reaches into a parent
// project to discover its own catalogue.
//
// Usage:
//
//	t := i18n.NoopTranslator{} // production default — returns key verbatim
//	msg := t.T("plugins_metadata_name_required", nil)
//
// Consuming projects (e.g. HelixCode) wire a real translator that
// loads pkg/i18n/bundles/active.en.yaml + locale overrides; the Plugins
...
// NoopTranslator is the zero-dependency production-safe default
// returned by package consumers that have not yet wired a project-side
// translator. It returns the key verbatim, which keeps the legacy
// error text shape ("plugins_metadata_name_required") rather than an
// empty string — actionable for downstream string assertions and
// visible in logs.
